...
This directory is a curated template copy of the workspace-local `nix-config/` repository.

Template notes:

- Source repo metadata such as `.git/` and `.codex/` was intentionally excluded.
- This is a real personal configuration, so some host names and machine-specific details are preserved as pattern examples rather than generalized placeholders.
- Use it as a reference for flake monorepo layout, host factoring, Home Manager split, overlays, and package extension patterns.
...
Use this tree for first-pass template selection. Only descend into specific subtrees after the layout direction is clearer.
