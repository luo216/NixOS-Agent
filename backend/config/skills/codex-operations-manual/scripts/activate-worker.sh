...
TARGET_CLIENT_ID="worker"
default_message="请监督执行当前需要的 nix/ssh/长时间命令，统一使用 tmux-session-runner，并持续回报状态、交互点与结果。"
message="${*:-$default_message}"

if [[ -z "$from_client_id" ]]; then
  echo "usage: activate-worker.sh <from-client-id> [message]" >&2
  exit 1
fi
